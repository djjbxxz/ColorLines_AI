...
from PPO.Env import ColorLineEnv
...
def main(args):
    ...
    train_env = TFPyEnvironment(BatchedPyEnvironment([ColorLineEnv()for _ in range(config.pop('env_batch_size'))]))
    test_env = TFPyEnvironment(ColorLineEnv())
    ...
    if not isDebug():
        agent.logger.save_config([args.agent_config])
