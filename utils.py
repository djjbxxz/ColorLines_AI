...
def convert_994_to_9928(game_map: np.ndarray) -> np.ndarray:
    new_game_map = np.zeros(
        shape=(config.BOARD_SIZE, config.BOARD_SIZE, 28), dtype=np.int32)
    for next_color in range(3):
        color = game_map[0, 0, next_color+1]
        if color:
            new_game_map[:, :, 6+next_color*7+color] = 1
    for x in range(config.BOARD_SIZE):
        for y in range(config.BOARD_SIZE):
            t = game_map[x, y, 0]
            if t != 0:
...
def pick_action_randomly_from_p(p: np.ndarray):
    assert p.shape == (config.POTENTIAL_MOVE_NUM,
                       ), f"Policy vector size should be {config.POTENTIAL_MOVE_NUM} instead of {p.size}"
    valid_move = np.argwhere(p)
    move_6561 = np.random.choice(valid_move.flatten())
    return convert_6561_to_coord(move_6561)
