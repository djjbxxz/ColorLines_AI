...
from sacd.model import DQNBase, TwinnedQNetwork, ActorNetwork
...
class SharedSacdAgent(BaseAgent):
    ...
    def __init__(self, env, test_env, log_dir, num_steps=100000, batch_size=64,
        ...
        # Define networks.
        self.conv = DQNBase(
            config.INPUT_CHANNEL_SIZE).to(self.device)
        self.policy = ActorNetwork(
            config.INPUT_CHANNEL_SIZE, config.POTENTIAL_MOVE_NUM,
    ...
    def load_models(self,p):
        self.conv.load(os.path.join(p,'conv.pth'))
        ...
        last_change_time = time.strftime("%Y/%m/%d %H:%M:%S",time.localtime(os.stat(os.path.join(p,'policy.pth')).st_mtime))
